...
public class Utilidades {
    ...
    public static UUID stringToUUID(String uuid) {
        ...
        return new UUID(Utilidades.bytesToLong(masSignificativo.getBytes()),
    ...
    public static int bytesToIntOK(byte[] bytes) {
        ...
        if (bytes.length > 4) throw new Error("demasiados bytes para pasar a int ");
    ...
    public static String bytesToHexString(byte[] bytes) {
        ...
        StringBuilder sb = new StringBuilder();
        for (byte b : bytes) {
            sb.append(String.format("%02X", b)); // Sin dos puntos y en mayúsculas
        }
        return sb.toString();
    }
}
